...
bool ParseShapeList(const std::string& text, std::vector<int>& shape) {
    ...
    while (std::getline(ss, token, ',')) {
        ...
        try {
            ...
            if (dim <= 0) {
                EDGE_LOG_ERROR("TensorRT dynamic shape dimension must be positive, got " << dim);
                return false;
            }
            shape.push_back(dim);
        } catch (const std::exception& ex) {
            EDGE_LOG_ERROR("Failed to parse TensorRT dynamic shape '" << text
                                                                      << "': " << ex.what());
            return false;
        }
    }
...
bool PaddleInferEngine::Init(const AppConfig& config) {
    ...
    if (config.paddle.use_gpu) {
        predictor_config.EnableUseGpu(config.paddle.gpu_mem_mb, config.paddle.gpu_device_id);
...
bool PaddleInferEngine::RunPredictor(const TensorBuffer& input) {
    ...
    try {
        ...
        if (!predictor_->Run()) {
            EDGE_LOG_ERROR("Paddle predictor Run() failed. input_shape="
        ...
        EDGE_LOG_ERROR("PaddleInferEngine exception during inference. input_shape="
                       << ShapeToString(input.shape)
                       << ", message=" << ex.what()
...
std::string PaddleInferEngine::Name() const {
    return "PaddleInferEngine";
}
...
bool PaddleInferEngine::ConfigureModel(
    ...
    const bool has_model_pair = !paddle_config.model_file.empty() && !paddle_config.params_file.empty();

    if (has_model_dir && has_model_pair) {
        EDGE_LOG_ERROR("Set either paddle.model_dir or paddle.model_file + paddle.params_file, not both");
        return false;
    }
    if (has_model_dir) {
    ...
    if (has_model_pair) {
        predictor_config.SetModel(paddle_config.model_file, paddle_config.params_file);
    ...
    EDGE_LOG_ERROR("PaddleInferEngine requires paddle.model_dir or paddle.model_file + paddle.params_file");
    return false;
}
...
bool PaddleInferEngine::ConfigureTensorRt(
    ...
    EDGE_LOG_INFO("TensorRT precision=" << trt_config.precision
                                        << ", max_batch_size="
                                        << trt_config.max_batch_size
                                        << ", min_subgraph_size="
                                        << trt_config.min_subgraph_size
                                        << ", workspace_size="
                                        << trt_config.workspace_size
                                        << ", use_static="
                                        << (trt_config.use_static ? "true" : "false")
                                        << ", use_calib_mode="
    ...
    if (!trt_config.enable) {
        EDGE_LOG_INFO("TensorRT disabled; using native Paddle Inference execution path");
        return true;
    }

    EDGE_LOG_INFO("[TRT_ANALYSIS] begin Paddle-TRT configuration snapshot");
    EDGE_LOG_INFO("[TRT_ANALYSIS] precision=" << trt_config.precision
    ...
    EDGE_LOG_INFO("[TRT_ANALYSIS] capture stdout/stderr and parse with scripts/parse_paddle_trt_log.py");
    ...
    if (trt_config.precision == "int8" && trt_config.use_calib_mode) {
        ...
        if (!EnsureDirectory(trt_config.calib_cache_dir, "TensorRT INT8 calibration cache dir")) {
            return false;
        }
        EDGE_LOG_INFO("TensorRT INT8 calibration mode enabled. Paddle will build calibration data "
                      "while running representative batches. Calibration image list="
                      << trt_config.int8_calib_images);
    } else if (!trt_config.cache_dir.empty()) {
        if (!EnsureDirectory(trt_config.cache_dir, "TensorRT optimized cache dir")) {
            return false;
        }
    }
    ...
    if (trt_config.enable_dynamic_shape) {
        ...
        if (min_shape.size() != opt_shape.size() || opt_shape.size() != max_shape.size()) {
        ...
        predictor_config.SetTRTDynamicShapeInfo(
            min_input_shape,
            max_input_shape,
            opt_input_shape,
    ...
    const std::string effective_cache_dir =
        (trt_config.precision == "int8" && trt_config.use_calib_mode && !trt_config.calib_cache_dir.empty())
            ? trt_config.calib_cache_dir
            : trt_config.cache_dir;
    if (!effective_cache_dir.empty()) {
        predictor_config.SetOptimCacheDir(effective_cache_dir);
        EDGE_LOG_INFO("TensorRT optimized/calibration cache dir=" << effective_cache_dir);
    }

    EDGE_LOG_INFO("TensorRT subgraph acceleration configured through Paddle Inference");
    return true;
}
...
bool PaddleInferEngine::CopyInput(const TensorBuffer& input) {
    ...
    if (input.shape.empty() || input.NumElements() == 0 || input.Data() == nullptr) {
    ...
    EDGE_LOG_INFO("PaddleInferEngine copied input name=" << input_names_.front()
                                                        << ", shape="
                                                        << ShapeToString(input.shape));
...
bool PaddleInferEngine::CopyOutput(TensorBuffer& output) {
    ...
    EDGE_LOG_INFO("PaddleInferEngine copied output name=" << output_names_.front()
                                                         << ", shape="
                                                         << ShapeToString(output.shape));
    if (output_names_.size() > 1) {
        EDGE_LOG_WARN("PaddleInferEngine currently reads only the first output tensor; extra outputs are ignored");
    }
    return true;
}
