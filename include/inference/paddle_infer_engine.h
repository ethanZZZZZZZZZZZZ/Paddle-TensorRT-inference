...
public:
    ...
    bool Infer(const DeviceTensorView& input, InferOutput& output) override;
...
private:
    ...
    bool RunPredictor(const DeviceTensorView& input);
    bool CopyInput(const TensorBuffer& input);
    bool BindDeviceInput(const DeviceTensorView& input);
