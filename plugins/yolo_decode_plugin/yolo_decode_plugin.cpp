...
constexpr int kSerializationVersion = 2;

template <typename T>
void WriteToBuffer(char*& buffer, const T& value) {
...
int YoloDecodePlugin::getNbOutputs() const noexcept {
    return kNbOutputs;
}
...
std::size_t YoloDecodePlugin::getWorkspaceSize(
    ...
    (void)outputs;
    (void)nb_outputs;
    if (inputs == nullptr || nb_inputs != kNbInputs || inputs[0].dims.nbDims != 3) {
        return 0;
    }
    return WorkspaceBytes(inputs[0].dims.d[0], top_k_);
}

int YoloDecodePlugin::enqueue(
    ...
    if (input_desc == nullptr || inputs == nullptr || outputs == nullptr ||
        ...
        outputs[0] == nullptr || outputs[1] == nullptr || workspace == nullptr) {
        return 1;
    }

    const auto& model_dims = input_desc[0].dims;
    const auto& meta_dims = input_desc[1].dims;
    if (model_dims.nbDims != 3 || meta_dims.nbDims != 2 ||
        model_dims.d[0] <= 0 || model_dims.d[1] <= 0 ||
        model_dims.d[2] <= 0 || meta_dims.d[1] != kMetaValues) {
        return 1;
    }
    ...
    const bool bcn_layout = model_dims.d[1] <= 256 && model_dims.d[2] > model_dims.d[1];
    ...
    workspace_cursor += static_cast<std::size_t>(batch) *
                        static_cast<std::size_t>(top_k_) *
                        static_cast<std::size_t>(kCandidateValues) *
                        sizeof(float);
    ...
    cudaError_t status = cudaMemsetAsync(pre_nms_candidates, 0, candidates_bytes, stream);
    ...
    status = bcn_layout
        ? LaunchDecodePreNMSBcnFloatMetaKernel(model_output,
                                               batch,
                                               channels,
                                               boxes,
                                               preprocess_metas,
                                               ...
                                               pre_nms_counts,
                                               stream)
        : LaunchDecodePreNMSFloatMetaKernel(model_output,
                                            batch,
                                            boxes,
                                            values_per_box,
                                            preprocess_metas,
    ...
    status = LaunchNMSFloatCandidatesKernel(pre_nms_candidates,
                                            pre_nms_counts,
                                            batch,
                                            top_k_,
                                            nms_threshold_,
...
int YoloDecodePlugin::initialize() noexcept {
    return 0;
}
...
std::size_t YoloDecodePlugin::getSerializationSize() const noexcept {
    return sizeof(int) + sizeof(float) + sizeof(float) + sizeof(int) + sizeof(int) + sizeof(int);
}
...
nvinfer1::IPluginV2DynamicExt* YoloDecodePlugin::clone() const noexcept {
    auto* plugin = new (std::nothrow) YoloDecodePlugin(
        score_threshold_,
        nms_threshold_,
        top_k_,
        input_width_,
...
void YoloDecodePlugin::destroy() noexcept {
    delete this;
}

void YoloDecodePlugin::setPluginNamespace(const char* plugin_namespace) noexcept {
...
bool YoloDecodePlugin::canBroadcastInputAcrossBatch(int input_index) const noexcept {
    (void)input_index;
    return false;
}
...
YoloDecodePluginCreator::YoloDecodePluginCreator() {
    ...
    fields_.emplace_back(
        nvinfer1::PluginField{"nms_threshold", nullptr, nvinfer1::PluginFieldType::kFLOAT32, 1});
    fields_.emplace_back(
        nvinfer1::PluginField{"top_k", nullptr, nvinfer1::PluginFieldType::kINT32, 1});
    fields_.emplace_back(
        nvinfer1::PluginField{"input_width", nullptr, nvinfer1::PluginFieldType::kINT32, 1});
    fields_.emplace_back(
        nvinfer1::PluginField{"input_height", nullptr, nvinfer1::PluginFieldType::kINT32, 1});
...
nvinfer1::IPluginV2* YoloDecodePluginCreator::deserializePlugin(
    ...
    (void)name;
    auto* plugin = new (std::nothrow) YoloDecodePlugin(serial_data, serial_length);
    if (plugin != nullptr) {
        plugin->setPluginNamespace(namespace_.c_str());
    }
    return plugin;
...
void YoloDecodePluginCreator::setPluginNamespace(const char* plugin_namespace) noexcept {
    namespace_ = plugin_namespace != nullptr ? plugin_namespace : "";
}
